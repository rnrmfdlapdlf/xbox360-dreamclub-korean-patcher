...
namespace DreamClubKoreanPatcher
{
    internal sealed class PatchPipeline
    {
        ...
        private void BuildRelocatedXex(
            ...
            string xexToolPath, string workDirectory)
        {
            ...
            if (newSection.VirtualAddress != virtualSize ||
                newSection.RawOffset != virtualSize)
            {
                throw new InvalidDataException(
                    ".kotext 위치가 XEX 이미지의 기존 데이터 끝과 일치하지 않습니다.");
            }

            int pointerCount = ReadReportInteger(
                Path.Combine(Path.GetDirectoryName(patchedPePath), "relocation_report.json"),
                "pointerPatchCount");
            int appended = pointerCount == 0 ? 0 : patchedPe.Length - virtualSize;
            if (appended < newSection.RawSize)
                throw new InvalidDataException(".kotext 데이터 영역이 올바르지 않습니다.");
            byte[] output = new byte[originalXex.Length + appended];
            Buffer.BlockCopy(originalXex, 0, output, 0, originalXex.Length);

            int pe = BitConverter.ToInt32(originalPe, 0x3C);
            int optional = pe + 24;
            int sizeOfHeaders = BitConverter.ToInt32(originalPe, optional + 60);
            CopyChangedPeRange(
                originalPe, patchedPe, output, blocks,
                0, 0, sizeOfHeaders);
            foreach (PeSection section in originalSections)
            {
                ...
                CopyChangedPeRange(
                    originalPe, patchedPe, output, blocks,
                    section.RawOffset, section.RawOffset, size);
            }
            if (appended != 0)
            {
                Buffer.BlockCopy(
                    patchedPe, virtualSize, output,
                    originalXex.Length, appended);
                BasicBlock last = blocks[blocks.Count - 1];
                if (last.ZeroSize != 0)
                    throw new InvalidDataException("마지막 XEX 블록에 생략 영역이 있습니다.");
                WriteBe32(output, last.DescriptorOffset, last.DataSize + appended);
            }
            Directory.CreateDirectory(Path.GetDirectoryName(outputPath));
            string intermediatePath = Path.Combine(
                workDirectory, "default_relocated_intermediate.xex");
            string verificationPe = Path.Combine(
                workDirectory, "default_relocated_verification.exe");
            try
            {
                File.WriteAllBytes(intermediatePath, output);
                RunProcess(
                    xexToolPath,
                    Quote("-e") + " " + Quote("u") + " " +
                    Quote("-c") + " " + Quote("u") + " " +
                    Quote("-o") + " " + Quote(outputPath) + " " +
                    Quote(intermediatePath),
                    workDirectory, false);
                RunProcess(
                    xexToolPath,
                    Quote("-b") + " " + Quote(verificationPe) + " " +
                    Quote(outputPath),
                    workDirectory, false);
            ...
            finally
            {
                ...
                if (File.Exists(verificationPe)) File.Delete(verificationPe);
            }
        }

        private static void CopyChangedPeRange(
            byte[] originalPe, byte[] patchedPe, byte[] outputXex,
            IEnumerable<BasicBlock> blocks,
            int originalOffset, int virtualOffset, int size)
        {
            for (int index = 0; index < size; ++index)
            {
                int raw = originalOffset + index;
                if (originalPe[raw] == patchedPe[raw]) continue;
                int xex = RvaToXexOffset(virtualOffset + index, blocks);
                if (xex < 0 || xex >= outputXex.Length)
                    throw new InvalidDataException("PE 변경 위치를 XEX에서 찾지 못했습니다.");
                if (outputXex[xex] != originalPe[raw])
                {
                    throw new InvalidDataException(
                        "PE와 XEX의 원본 데이터가 일치하지 않습니다: 0x" +
                        raw.ToString("X8"));
                }
                outputXex[xex] = patchedPe[raw];
            }
        }

        private static void VerifyRelocatedPe(
            byte[] expectedPe, string actualPePath, PeSection expectedSection)
        {
            byte[] actualPe = File.ReadAllBytes(actualPePath);
            PeSection actualSection = ParsePeSections(actualPe)
                .FirstOrDefault(section => section.Name == ".kotext");
            if (actualSection == null ||
                actualSection.VirtualAddress != expectedSection.VirtualAddress ||
                actualSection.VirtualSize != expectedSection.VirtualSize ||
                actualSection.RawSize < expectedSection.VirtualSize)
            {
                throw new InvalidDataException(
                    "재구성된 XEX에서 .kotext 섹션을 확인하지 못했습니다.");
            }
            for (int index = 0; index < expectedSection.VirtualSize; ++index)
            {
                if (expectedPe[expectedSection.RawOffset + index] !=
                    actualPe[actualSection.RawOffset + index])
                {
                    throw new InvalidDataException(
                        "재구성된 XEX의 .kotext 데이터가 일치하지 않습니다.");
                }
            }
        }
